...
<script type="text/javascript">
  ...
  function dohihi(data, site) {
    myChart.setOption(option = {
      title: {
        text: site + '：' + data['newAQI'].NO2 + ', ' + data['newAQI'].PublishTime,
        textStyle: {
          fontSize: 18
        }
      },
      ...
      visualMap: {
        ...
        right: -100,
        pieces: [
          {gt: 0, lte: 53, color: '#2FC737'},
          {gt: 53, lte: 100, color: '#E6E600'},
          {gt: 100, lte: 360, color: '#FF7C19'},
          {gt: 360, lte: 649, color: '#F33515'},
          {gt: 649, lte: 1249, color: '#BC1049'},
          {gt: 1249, color: '#800000'}
        ],
        outOfRange: {
          color: '#999'
        }
        ...
        data.map(function(item) {
          var marker = new google.maps.Marker({
            ...
            icon: {
              ...
              fillColor: changeByNO2(item.NO2),
              fillOpacity: 0.7,
              strokeColor: changeByNO2(item.NO2),
              strokeWeight: 0,
              scale: 15,
            },
          ...
          marker.addListener('mouseover', function() {
            this.setIcon({
              path: google.maps.SymbolPath.CIRCLE,
              fillColor: changeByNO2_over(item.NO2),
              fillOpacity: 0.7,
              strokeColor: changeByNO2_over(item.NO2),
              strokeWeight: 0,
              scale: 15
            });
          });
